...
def setup():
    ...
    print(PFont.list())
    roboto12 = createFont("Helvetica-Bold", 12)
    roboto16 = createFont("Helvetica-Bold", 16)
...
def draw():
    ...
    startDate = drawTask(currentX, currentY, rect_width_per_day, taskRectHeight, startDate_of_chart, 
        startDate, 4, "Test")
    currentY = currentY + taskOffset # Increment Y-coordinate to draw next task rectangle
    startDate = drawTask(currentX, currentY, rect_width_per_day, taskRectHeight, startDate_of_chart, 
        startDate, 2, "Test")
    currentY = currentY + taskOffset
    startDate = drawTask(currentX, currentY, rect_width_per_day, taskRectHeight, startDate_of_chart, 
        startDate, 9, "Test")
    currentY = currentY + taskOffset
    startDate = drawTask(currentX, currentY, rect_width_per_day, taskRectHeight, startDate_of_chart, 
...
def drawTask(startX, startY, rect_width_per_day, rectHeight, chart_start, task_start_date, task_duration_in_days, title):
    days_from_start = abs(chart_start - task_start_date)
    x = (days_from_start.days * rect_width_per_day) + startX
    rectWidth = task_duration_in_days * rect_width_per_day
    fill(0, 255, 255)
    ...
    text(title, title_start_x, startY, textOffset + 10, rectHeight)
    textSize(10)
    textAlign(RIGHT, CENTER)
    dateText = task_start_date.strftime("%m/%d")
    textOffset = textWidth(dateText) + 10
    text(dateText, x - (textOffset + 2), startY, textOffset, rectHeight)
